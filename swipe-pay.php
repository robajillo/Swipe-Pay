...
//$credentials = $response_obj->data->credentials;
//$global_key = $response_obj->data->{'Global-key'};
//$server_url = $response_obj->data->{'Server-url'};
...
//Form Data
$request_id="ws_CO_040620200606088489";
//$phone ="25471396806";//or could be $_POST['phone']; if you're collecting information from a form. :-) Simple huh?
...
//   $response_from_api = curl_exec($curl);
  
//   curl_close($curl);
  
//   //Parse JSON response
//   $response_obj = json_decode($response_from_api);
...
//   > 
 ...
 curl_setopt_array($curl, array(
    CURLOPT_URL => "https://www.api.swypepay.africa/v3/apis/validate-mpesa-stk",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => "",
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 0,
    ...
    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
    CURLOPT_CUSTOMREQUEST => "POST",
    CURLOPT_POSTFIELDS => array('request_id' => $request_id),
    CURLOPT_HTTPHEADER => array(
      "Global-key: $global_key",
      "Server-url: $server_url",
      "Access-Token: $token"
  ...
  $validate_mpesa = curl_exec($curl);
  
  curl_close($curl);
  
  //Parse JSON response
  $response_obj = json_decode($validate_mpesa);
  //Get back iframe link
  $reference_code= $response_obj->data->reference_code;
